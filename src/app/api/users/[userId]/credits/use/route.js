...
export async function POST(request, { params }) {
  try {
    ...
    if (users[userIndex].credits < creditsAmount) {
      return NextResponse.json(
        {
          success: false,
          message: "Insufficient credits",
          availableCredits: users[userIndex].credits,
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      ...
      message: `${creditsAmount} credits used successfully`,
    });
  } catch (error) {
    console.error("Error in use credits route:", error);
    return NextResponse.json(
